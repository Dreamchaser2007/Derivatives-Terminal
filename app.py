...
def get_ohlc_for_symbol(symbol: str, period: str = "1mo"):
    ...
    try:
        ...
        if df is None or df.empty:
            try:
                df = yf.download(
                    tickers=symbol, period=period, interval="1d", threads=False, progress=False
                )
            except Exception:
                df = None

        if (df is None or df.empty) and (not is_fx_symbol(symbol)):
            try:
                stooq_symbol = symbol.lower()
                if not re.search(r"\.[a-z]{1,3}$", stooq_symbol):
                    stooq_symbol = f"{stooq_symbol}.us"
                stooq_url = f"https://stooq.com/q/d/l/?s={stooq_symbol}&i=d"
                resp = requests.get(stooq_url, timeout=10, headers={"User-Agent": "Mozilla/5.0"})
        ...
        if df is not None and hasattr(df.columns, "nlevels") and df.columns.nlevels > 1:
            top_levels = list(dict.fromkeys(df.columns.get_level_values(0)))
            if symbol in top_levels:
                df = df[symbol]
            else:
                df = df[top_levels[0]]

        if df is None or df.empty:
            print(f"get_ohlc_for_symbol: no data for {symbol}")
            return None

        def _apply_period_filter(df_local, period_str):
            if not period_str:
                return df_local
            ...
            try:
                if unit.startswith("d"):
                    cutoff = now - pd.Timedelta(days=num)
                elif unit in ("mo", "m", "mon", "month", "months"):
                    cutoff = now - pd.DateOffset(months=num)
                elif unit in ("y", "yr", "yrs", "year", "years"):
                    cutoff = now - pd.DateOffset(years=num)
                else:
                    return df_local
                return df_local[df_local.index >= cutoff]
            except Exception:
                return df_local
        ...
        if df.empty:
            print(f"get_ohlc_for_symbol: all-NaN after dropna for {symbol}")
            return None
...
def get_movers(universe: List[str] = SAMPLE_UNIVERSE):
    ...
    try:
        data = yf.download(
            tickers=" ".join(stock_universe), period="5d", interval="1d", group_by="ticker", threads=False, progress=False
        )
...
def bs_greeks(S, K, r, sigma, T, q=0.0):
    ...
    theta_call = (
        -S * sigma * math.exp(-q * T) * norm_pdf(d1) / (2 * math.sqrt(T))
            ...
            if idx != -1:
                ...
                if start != -1 and end != -1:
                    ...
                    try:
                        ...
                        data = None
        if data:
            options_obj = _recursive_find(data, "optionChain") or _recursive_find(data, "options") or _recursive_find(
                data, "optionContracts"
            )
            if options_obj is None:
                stores = _recursive_find(data, "stores") or data
                options_obj = (
                    _recursive_find(stores, "OptionChainStore")
                    or _recursive_find(stores, "OptionContractsStore")
                    or _recursive_find(stores, "OptionStore")
                )
            results = []
...
def index():
    ...
    if request.method == "POST":
        ...
        if tab_type == "chart":
            active_tab = "chart"
            symbol = sanitize_symbol(request.form.get("chart_symbol", chart_form["symbol"]))
            ...
            if fx_form["use_live_price"]:
                try:
                    ohlc = get_ohlc_for_symbol(fx_form["symbol"], period="5d")
                    if ohlc and ohlc.get("close"):
                        S_used = float(ohlc["close"][-1])
                except Exception:
                    S_used = None
            ...
            if eq_form["use_live_price"]:
                try:
                    ohlc = get_ohlc_for_symbol(eq_form["symbol"], period="5d")
                    if ohlc and ohlc.get("close"):
                        S_used = float(ohlc["close"][-1])
                except Exception:
                    S_used = None
            ...
            if S_used is None:
                ...
                try:
                    if model == "bs":
                        ...
                        g = bs_greeks(S_used, K, r, sigma, T, q=q)
                        greeks = {"delta": g["delta_call"], "gamma": g["gamma"], "vega": g["vega"], "theta": g["theta_call"], "rho": g["rho_call"]}
                    eq_result = {"model_label": {"bs": "Black–Scholes", "binomial_eur": "Binomial (European)", "binomial_amer": "Binomial (American)"}[model], "price": float(price), "S_used": float(S_used), "greeks": greeks}
                    s_range = np.linspace(S_used * 0.8, S_used * 1.2, 30).tolist()
                    price_arr = []
                    delta_arr = []
                    gamma_arr = []
            ...
            active_tab = "fx_chain"
            fx_chain_form["root"] = sanitize_symbol(request.form.get("fx_chain_root", ""))
            fx_chain_form["contract"] = sanitize_symbol(request.form.get("fx_chain_contract", ""))
            ...
            eq_chain_form["symbol"] = sanitize_symbol(request.form.get("eq_chain_symbol", ""))
            ...
            if scenario_form["use_live_price"]:
                try:
                    ohlc = get_ohlc_for_symbol(scenario_form["symbol"], period="5d")
                    if ohlc and ohlc.get("close"):
                        S_used = float(ohlc["close"][-1])
            ...
            if S_used is None:
                ...
                try:
                    ...
                    scenarios = scenario_analysis(S_used, K, scenario_form["r"], scenario_form["sigma"], scenario_form["T"], scenario_form["option_type"], scenario_form["model"], scenario_form["q"])
            ...
            if comparison_form["use_live_price"]:
                try:
                    ohlc = get_ohlc_for_symbol(comparison_form["symbol"], period="5d")
                    if ohlc and ohlc.get("close"):
                        S_used = float(ohlc["close"][-1])
            ...
            if S_used is None:
                ...
                try:
                    ...
                    comparison = compare_models(S_used, K, comparison_form["r"], comparison_form["sigma"], comparison_form["T"], comparison_form["option_type"], comparison_form["q"], steps=comparison_form["steps"])
    ...
    return render_template(
        ...
        eq_chart=eq_chart,
        eq_error=eq_error,
        fx_chain_form=fx_chain_form,
        fx_chain_calls=fx_chain_calls,
        fx_chain_puts=fx_chain_puts,
        eq_chain_form=eq_chain_form,
        eq_chain_calls=eq_chain_calls,
        eq_chain_puts=eq_chain_puts,
